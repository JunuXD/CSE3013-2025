...
void InitTetris(){
	...
	if(MODIFIED_FLAG){
		...
		compress_field(field, mRecRoot->recField);
...
void DrawEmpty(int blockNum, int blockRotate, int blockY, int blockX){
	int i, j;
	for(i = 0; i < 4; i++){
		for(j = 0; j < 4; j++){
			if(block[blockNum][blockRotate][i][j]==1 && i+blockY>=0){
...
void DrawChange(char f[HEIGHT][WIDTH],int command,int currentBlock,int blockRotate, int blockY, int blockX){
	...
	DrawEmpty(currentBlock, prevRotate, prevY, prevX);
	...
	DrawEmpty(currentBlock, prevRotate, shadowY, shadowX);
	DrawEmpty(currentBlock, recommendR, recommendY, recommendX);
...
int mAddBlockToField(char f[HEIGHT][WIDTH], int currentBlock, int blockRotate, int blockY, int blockX){
	int i, j, ret1 = 0, ret2 = 0, ret3 = 0, ls = -1;
	...
	for(i = 0; i < 4; i++){
		for(j = 0; j < 4; j++){
			...
			if(blockY + i + 1 >= HEIGHT || f[blockY + i + 1][blockX + j] == 1) ret1++;
			if(blockX + j + 1 >= WIDTH) ret2++; 
			else if(f[blockY + i][blockX + j + 1]) ret3++;

			if(blockX + j - 1 < 0) ret2++;
			else if (j && f[blockY + i][blockX + j - 1]) ret3++;

			f[blockY + i][blockX + j] = 1;
		}
	}

	return ret1 * 9 + ret2 * 5 + ret3 * 7;
}
...
void compress_field(char field[HEIGHT][WIDTH], int cfield[WIDTH]){
...
void decompress_field(int cfield[WIDTH], char field[HEIGHT][WIDTH]){
	for(int i = 0; i < WIDTH; i++){
		for(int j = 0; j < HEIGHT; j++){
			field[j][i] = ((cfield[i] >> j) & 1);
		}
	}
}
...
int modified_recommend(mRecNode *root){
	...
	int currBlock = nextBlock[(root->level) - 1];
	char f[HEIGHT][WIDTH];
	int rotates = NUM_OF_ROTATE;

	if(currBlock == 0) rotates = 1;
	else if (currBlock == 4 || currBlock == 5 || currBlock == 6) rotates = 2;
	...
	for(r = 0; r < rotates; r++){
		for(x = -BLOCK_WIDTH; x <= WIDTH; x++){
			...
			decompress_field(root->recField, f);

			while(CheckToMove(f, currBlock, r, y + 1, x)){
				y++;
			}
			...
			if(rec_play) spacet++;
			mRecNode *c = (mRecNode*) malloc(sizeof(mRecNode));
			root->child[i++] = c;

			// accumulate child's Score
			c->accumulatedScore = root->accumulatedScore;
			c->accumulatedScore += mAddBlockToField(f, currBlock, r, y, x);
			c->accumulatedScore += DeleteLine(f);

			c->level = (root->level) + 1;

			if(c->level <= BLOCK_NUM){
				compress_field(f, c->recField);
				curr = modified_recommend(c);
...
void recommend_play_step(int sig){
	timed_out = 0;
	...
	timet += (double)difftime(stop, start);

	score += AddBlockToField(field, nextBlock[0], recommendR, recommendY, recommendX);
	score += DeleteLine(field);
	PrintScore(score);
	
	DrawEmpty(nextBlock[0], blockRotate, blockY, blockX);
...
void recommendedPlay(){
	timet = 0, spacet = 0, rec_play = 1;

	clear();
	act.sa_handler = recommend_play_step;
	sigaction(SIGALRM,&act,&oact);
	...
	spacet *= ((MODIFIED_FLAG) ? sizeof(mRecNode): sizeof(RecNode));

	printw("t : %f\n", (double)difftime(stop, start));
	printw("score(t) : %d\n", score);
	printw("time(t) : %f\n", timet);
	printw("space(t) : %d bytes\n", spacet);

	printw("\ntime eff : %f\n", (double)score / timet);
	printw("space eff : %f", (double)score / spacet);
